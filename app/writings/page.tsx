    ...
    <>
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-10 flex items-center justify-between">
          <h1 className="font-serif text-5xl">Featured Writings</h1>
          <Link href="/" className="text-sm text-primary">← Back home</Link>
        </div>

        {pagedItems.length === 0 ? (
          <p className="rounded-xl border border-dashed border-border p-8 text-center text-muted-foreground">No writing entries yet.</p>
        ) : (
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {pagedItems.map((item) => (
              <article key={item.id} className="rounded-2xl border border-border bg-card p-5 shadow-md transition-shadow duration-200 hover:shadow-lg">
                ...
                <p className="text-xs uppercase tracking-[0.12em] text-primary">{item.category || "General"}</p>
                <h2 className="mt-2 font-serif text-2xl">{item.title}</h2>
                <p className="mt-3 text-sm leading-7 text-muted-foreground line-clamp-4">{item.desc}</p>
                <a href={item.readUrl || "#"} target="_blank" rel="noopener noreferrer" className="mt-4 inline-block text-sm font-medium text-primary">Read article →</a>
              </article>
            ))}
          </div>
        )}
